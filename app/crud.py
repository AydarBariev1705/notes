...
def get_notes(db: Session, user_id: int, skip: int = 0, limit: int = 10, tag: str = None):
    query = db.execute(select(Note).filter(Note.user_id == user_id).offset(skip).limit(limit))
    if tag:
        tag_obj = db.execute(select(Tag).filter(Tag.name == tag)).scalars().first()
        if not tag_obj:
            return None
        query = db.execute(select(Note).join(Note.tags).filter(Tag.id == tag_obj.id).filter(Note.user_id == user_id).offset(skip).limit(limit))
...
def update_note_tags(db: Session, db_note: Note, new_tags: set):
...
def search_notes_by_tag(db: Session, user_id: int, tag: str):
    return db.execute(
